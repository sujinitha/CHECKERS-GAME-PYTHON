...
#   given position of a piece (column, row) it would return all move both allowed and not allowed
    ...
    def removePiece(self, pieceCoords):
        self.boardStat[int(pieceCoords[1])][int(pieceCoords[0])] = (black, pieceStat(None))
...
class game:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                if self.goOver == False:
                    ...
                    elif self.clicked != None and self.simpleMousePos in self.board.availableMoves(self.clicked[0],self.clicked[1], self.goOver):
                        self.board.movePiece(self.clicked,self.simpleMousePos)
                        #   check if an attack move was made
                        if self.simpleMousePos not in self.board.diagonalSquares(self.clicked[0],self.clicked[1]):
                ...
                #   if self.goOver == True (an attack move was made)
                    ...
                    availMoves = self.board.availableMoves(self.clicked[0],self.clicked[1],self.goOver)
            ...
            #   self.allowedMoves so that a yellow circle would be drawn to represent available move
                ...
                self.allowedMoves = self.board.availableMoves(self.clicked[0],self.clicked[1], self.goOver)
    ...
    def getAiAvailMoves(self,pieces):
        ...
        for pieceCoords in pieces:
            availMoves = self.board.availableMoves(pieceCoords[0],pieceCoords[1], self.goOver)
            if availMoves != []:
    ...
    def AiMove(self):
        ...
        if piecesPos != []:
            ...
            if self.goOver == False:
                ...
                #   check if an attack move was made
                if move not in self.board.diagonalSquares(pieceCoords[0],pieceCoords[1]):
                    x = pieceCoords[0] + (move[0] - pieceCoords[0]) / 2
                    y = pieceCoords[1] + (move[1] - pieceCoords[1]) / 2
                    pieceCoordss=(x,y)
                    self.board.removePiece(pieceCoordss)
                    self.updateGameDisplay()
                    self.goOver = True
                    pieceCoords = move
    ...
    def move(self,fromCoords, toCoords):
        ...
        if toCoords not in self.board.diagonalSquares(fromCoords[0],fromCoords[1]):
    ...
    def undoMove(self,fromPos,fromStat,toPos, enemyRemoved):
        ...
        if enemyRemoved != None:
            self.board.boardStat[int(enemyRemoved[1])][int(enemyRemoved[0])] = (enemyRemoved[2],enemyRemoved[3])
    ...
    def ifJump(self,fromCoords, toCoords):
        if toCoords not in self.board.diagonalSquares(fromCoords[0],fromCoords[1]):
            x = fromCoords[0] + (toCoords[0] - fromCoords[0]) / 2
            y = fromCoords[1] + (toCoords[1] - fromCoords[1]) / 2
            removedColor = self.board.boardStat[int(y)][int(x)][0]
            removedStat = self.board.boardStat[int(y)][int(x)][1]
    ...
    def hardAi(self):
        ...
        if self.goOver == False:
            ...
            if move not in self.board.diagonalSquares(piece[0],piece[1]):
    ...
    def resetAi(self):
        ...
        if self.checkSomeoneWon():
            if self.AiTurn == True:
                self.wnd.winMessage("You Won!")
            else:
                self.wnd.winMessage("AI Won!")
